...
import { Clock, Calendar, Glasses, CheckCircle2, Armchair, Mail } from "lucide-react"
...
export default function ThreeDPage() {
  ...
  const [selectedMovie, setSelectedMovie] = useState<(typeof threeDMovies)[0] | null>(null)
  ...
  const handleBookClick = (movie: (typeof threeDMovies)[0]) => {
    if (user.role === "guest") {
  ...
  const handleConfirmBooking = () => {
    if (selectedMovie && selectedShowtime && selectedDate && selectedSeat) {
      addTicket({
        ...
        date: selectedDate,
        theater: `3D Theater ${Math.floor(Math.random() * 4) + 1}`,
        seat: selectedSeat,
        screen: "3D Screen",
    ...
    <div className="min-h-screen bg-background">
      ...
      <Dialog open={showBookingDialog} onOpenChange={setShowBookingDialog}>
        <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Book 3D Ticket</DialogTitle>
            <DialogDescription>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-center space-y-2">
              <p>Your 3D ticket for {selectedMovie?.title} has been successfully booked.</p>
